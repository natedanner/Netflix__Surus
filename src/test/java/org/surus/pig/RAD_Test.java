...
public class RAD_Test {
    ...
    public void testNormal() throws Exception {
        ...
        DataBag expected  = bf.newDefaultBag();
        {
            ...
            expected = buildDataBag(ts,lR,sR,eR,false);
    ...
    private Boolean approximateCompareBags(DataBag inputBag1, DataBag inputBag2) throws ExecException {
		...
		while (iter1.hasNext()) {
			...
			// Check error
			if (Math.abs((Double) tuple1.get(0) - (Double) tuple2.get(0)) > errorLimit) {
				return false;
			}
			// TODO: Add unit test for differenced case
			//if (Math.abs((Double) tuple1.get(1) - (Double) tuple2.get(1)) > errorLimit) return false;
			if (Math.abs((Double) tuple1.get(2) - (Double) tuple2.get(2)) > errorLimit) {
				return false;
			}
			if (Math.abs((Double) tuple1.get(3) - (Double) tuple2.get(3)) > errorLimit) {
				return false;
			}
			if (Math.abs((Double) tuple1.get(4) - (Double) tuple2.get(4)) > errorLimit) {
				return false;
			}
    ...
    private Schema buildInputSchema2() {
		...
		// Outer Tuple Schema
    	List<FieldSchema> fieldSchemaFinal = new ArrayList<>();
        try {
        ...
        // Return Schema
		return new Schema(fieldSchemaFinal);
